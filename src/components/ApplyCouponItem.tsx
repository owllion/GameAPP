...
interface Props {
  ...
  finalTotal: number;
  discount: number;
  setCodeHandler: (text: string) => void;
  clearCode: () => void;
  code: string;
}

const ApplyCouponItem = (props: Props) => {
  const { finalPrice, finalTotal, discount, setCodeHandler, clearCode, code } =
    props;
    ...
    <>
      <OrderDetailBox>
        <CodeInputBox>
          <CodeInput
            value={code}
            onChangeText={(c: string) => setCodeHandler(c)}
...
color:${({ highlight }: { highlight: boolean }) =>
  highlight ? COLORS.orange : COLORS.light}
font-family:IBMPlexSansRegular;
